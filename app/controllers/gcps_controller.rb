class GcpsController < ApplicationController
  layout 'application'
  #skip_before_action :verify_authenticity_token, :only => [:update, :update_field, :add, :destroy, :show]

  before_action :authenticate_user!, :only => [:update, :update_field, :add, :destroy]
