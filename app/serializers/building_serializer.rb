class BuildingSerializer
  ...
  attribute :census_records do |object, params|
    if params[:condensed]
      ...
      {
        1900 => object.census_1900_records.andand.map { |item| CensusRecordSerializer.new(item).as_json },
        1910 => object.census_1910_records.andand.map { |item| CensusRecordSerializer.new(item).as_json },
        1920 => object.census_1920_records.andand.map { |item| CensusRecordSerializer.new(item).as_json },
        1930 => object.census_1930_records.andand.map { |item| CensusRecordSerializer.new(item).as_json }
      }
    end
  end
end
